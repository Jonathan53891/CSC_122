...
public:
	...
	Experiment() : exp_name(), creature(), population(), 
		       tr1_name(), tr2_name(), midtr_name("Middle Trait"), 
		       tr1_count(), tr2_count() {}
	...
	void write(std::ostream& out) /* writes data to file */
	{
		...
		/* Calculations */
		double  tr1_perc = static_cast<double> (tr1_count + midtr_count / 2.0)*100/ population;
		double  tr2_perc = static_cast<double> (tr2_count + midtr_count / 2.0)*100/ population;
		...
		/* Trait 1 */
		out << right << setw(name_length) << tr1_name << ':' << string(spacing, ' ');
		out << right << setw(count_length) << tr1_count << string(spacing, ' ') ;
		out << right << setw(perc_length) << setprecision(2)<< fixed << tr1_perc << "%\n",
		
		/* Mixed Trait */
		out << right << setw(name_length) << midtr_name << ':' << string(spacing, ' ');
		out << right << setw(count_length) << midtr_count << endl;
		
		/* Trait 2 */
		out << right << setw(name_length) << tr2_name  << ':' << string((spacing-2), ' ') << '+' << ' ';
		out << right << setw(count_length) << tr2_count << string(spacing, ' ') ;
		out << right << setw(perc_length) << setprecision(2)  << fixed << tr2_perc << "%\n" ;
		
		/* Equals line under data */
		out << right << string((name_length+ spacing-1), ' ') << string(count_length +2, '-') << string(spacing, ' ') << string(7, '-') << endl;
		
		/* Total */
		out << right << setw(name_length) << "Total" << ':' << string(spacing, ' ');
		out << right << setw(count_length) << population << string(spacing, ' ');
		out << right << setw(perc_length) << setprecision(2) << fixed << tr1_perc +tr2_perc << "%\n\n\n";
	}
};
